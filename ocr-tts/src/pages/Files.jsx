...
export default function Files() {
  ...
  const renderConvertColumn = (file) => {
    ...
    if (conversion.status === 'completed') {
      // Completed - show play, download link and convert again option
      return (
        <div className="flex flex-col gap-1">
          <Link
            to={`/player/${file.file_id}`}
            className="text-green-600 hover:text-green-900 flex items-center gap-1 px-2 py-1 text-sm"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {files.map((file) => (
              <div key={file.file_id} className="bg-white shadow rounded-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {conversions[file.file_id]?.status === 'completed' && (
                    <Link
                      to={`/player/${file.file_id}`}
                      className="flex-1 min-w-[100px] text-center px-3 py-2 text-sm text-green-600 bg-green-50 hover:bg-green-100 rounded flex items-center justify-center gap-1 transition-colors"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
